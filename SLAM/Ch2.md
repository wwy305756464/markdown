...
* 这些时刻机器人的自身位置：x1, x2, ..., xK
* 地图上一共有N个路标点，每个时刻测潦倒的路标点数量：y1, y2, ..., yN
...
* 观测：假设机器人在 K 时刻于 xK 位置探测到了某一个路标 yj， 那么如何用数学语言来描述？
...
描述机器人在 xk 位置上看到某个路标点 yj 时，产生了一个观测数据 z_{k,j}
$$
...
$$
这里x1, x2是两个轴上的位置而 \theta 为转角。同时两个时间点运动的变化量为：
$$
